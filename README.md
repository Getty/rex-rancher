...
`Rex::GPU` is required only when using `gpu => 1` with the default `gpu_setup => 1`; with `gpu_setup => 0` (e.g. the NVIDIA GPU Operator provides driver and toolkit) it is not needed, and `gpu_device_plugin => 0` skips the device plugin as well. With `Rex::GPU` 0.002 (the recommended version), the GPU generation decides both whether a GPU gets a driver and which one: every Maxwell-or-newer GPU does, consumer and laptop cards (GeForce MX, GT 1030, GTX 9xx, laptop RTX) included, so a re-deploy with `gpu => 1` on such a node now installs the driver and, with `reboot => 1`, reboots it. Kepler and older GPUs are skipped with a warning; a Kepler-only node deploys without `nvidia.com/gpu`, and the device-plugin step ends with a warning. Conflicting GPU mixes such as V100 + B200 and a missing package source for the required driver make the deploy die after node preparation, before the driver or Kubernetes is installed; so does an NVIDIA vGPU guest (Azure NVadsA10 v5, AWS G6f, ...) without an already working licensed vGPU driver, even next to a passed-through GPU. HGX A100/H100/H200 nodes (NVSwitch) get NVIDIA Fabric Manager at exactly the driver's version; HGX B200/B300 get Fabric Manager plus the NVLink Subnet Manager `nvlsm`, `infiniband-diags`, `libibumad` and the `ib_umad` module, a warning on a kernel older than 5.17, and a warning if the fabric does not come up (OFED is not set up). Where no Fabric Manager or `nvlsm` source is known — Debian 11 or openSUSE with an NVSwitch, B200/B300 on an OS without an `nvlsm` source — the deploy dies at the same point, before any driver is installed. GB200/GB300 get an `nvidia-imex` hint — see "GPU hardware support" in the `Rex::Rancher` POD.

**Verifying a deploy:** `prove -lr t/` runs a compile check and offline unit tests only — there is no integration test. A pipeline change is only proven by a real deploy on a fresh node; [`eg/hetzner-gpu.Rexfile`](eg/hetzner-gpu.Rexfile) shows a complete one.
